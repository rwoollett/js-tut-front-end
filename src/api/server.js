...
import {seed, date, name, internet} from 'faker'
...
new Server({
  routes() {
    ...
    this.get('/notifications', (schema, req) => {
      ...
      const notifications = [...Array(numNotifications)].map(() => {
        ...
        return {
          id: nanoid(),
          date: date.between(pastDate, now).toISOString(),
          message: template,
          user: user.id,
          read: false,
          isNew: true,
  ...
  factories: {
    user: Factory.extend({
      ...
      name() {
        return name.findName(this.firstName, this.lastName)
      },
      username() {
        return internet.userName(this.firstName, this.lastName)
      },
